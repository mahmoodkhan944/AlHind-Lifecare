    ...
    <form onSubmit={handleSubmit} className="min-h-screen bg-gray-50 -m-4 sm:-m-6 lg:-m-8">
      <div className="sticky top-0 z-20 bg-white border-b border-gray-200 px-4 sm:px-6 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-gray-500">{isEdit ? "Update hospital details" : "Create a new hospital listing"}</p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6 space-y-4 pb-28">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-5 shadow-sm">
          <h3 className="font-bold text-gray-900 text-sm mb-4">Hospital Information</h3>
          <div className="space-y-3">
            {/* Image Upload */}
            <Field label="Hospital Image">
              {form.cover_image_url ? (
                <div className="relative">
                  <img src={form.cover_image_url} alt="Hospital" className="w-full h-40 rounded-lg object-cover border border-gray-200" />
                ...
                <label className="cursor-pointer block">
                  ...
                  <div className="flex flex-col items-center justify-center gap-1.5 py-8 rounded-lg border-2 border-dashed border-gray-300 hover:bg-gray-50 transition-colors">
                    {uploading ? <Loader2 className="w-7 h-7 text-gray-400 animate-spin" /> : <ImageIcon className="w-7 h-7 text-gray-300" />}
                    <p className="text-sm font-medium text-gray-600">{uploading ? "Uploading..." : "Click to upload or drag and drop"}</p>
            ...
            <Field label="Hospital Name" required>
              <Input value={form.name || ""} onChange={(e) => set("name", e.target.value)} placeholder="Enter hospital name" className="h-10 rounded-lg border-gray-200" />
            </Field>
            <Field label="Hospital Type">
              <Select value={form.hospital_type || ""} onValueChange={(v) => set("hospital_type", v)}>
                <SelectTrigger className="h-10 rounded-lg border-gray-200"><SelectValue placeholder="Select hospital type" /></SelectTrigger>
                <SelectContent>
            ...
            <Field label="Full Address" required>
              <Input value={form.address || ""} onChange={(e) => set("address", e.target.value)} placeholder="Enter complete address" className="h-10 rounded-lg border-gray-200" />
            </Field>
            <div className="grid grid-cols-2 gap-3">
              <Field label="City" required>
                <Input value={form.city || ""} onChange={(e) => set("city", e.target.value)} placeholder="Enter city" className="h-10 rounded-lg border-gray-200" />
              </Field>
              <Field label="State" required>
                <Input value={form.state || ""} onChange={(e) => set("state", e.target.value)} placeholder="Enter state" className="h-10 rounded-lg border-gray-200" />
              </Field>
            </div>
            <Field label="Google Maps Embed URL">
              <Input value={form.google_maps_embed_url || ""} onChange={(e) => set("google_maps_embed_url", e.target.value)} placeholder="Paste Google Maps embed URL here" className="h-10 rounded-lg border-gray-200" />
            ...
            <div className="grid grid-cols-2 gap-3">
              <Field label="Number of Beds" required>
                <Input type="number" value={form.beds_count ?? ""} onChange={(e) => set("beds_count", e.target.value)} placeholder="0" className="h-10 rounded-lg border-gray-200" />
              </Field>
              <Field label="Year Established" required>
                <Input type="number" value={form.established_year ?? ""} onChange={(e) => set("established_year", e.target.value)} placeholder="2026" className="h-10 rounded-lg border-gray-200" />
              </Field>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <Field label="Rating (0-5)">
                <Input type="number" step="0.1" max="5" value={form.rating ?? ""} onChange={(e) => set("rating", e.target.value)} placeholder="0" className="h-10 rounded-lg border-gray-200" />
              </Field>
              <Field label="Review Count">
                <Input type="number" value={form.reviews_count ?? ""} onChange={(e) => set("reviews_count", e.target.value)} placeholder="0" className="h-10 rounded-lg border-gray-200" />
              </Field>
            </div>
            <Field label="Short Description" required>
              <Textarea value={form.description || ""} onChange={(e) => set("description", e.target.value)} placeholder="Enter a brief description (1-2 sentences)" className="rounded-lg border-gray-200 min-h-[80px]" rows={3} />
            </Field>
            <Field label="Hospital Owner / Chairman">
              <Input value={form.hospital_owner || ""} onChange={(e) => set("hospital_owner", e.target.value)} placeholder="e.g., Dr. Prathap C Reddy" className="h-10 rounded-lg border-gray-200" />
            </Field>
            ...
            <div className="space-y-2 pt-2 border-t border-gray-100">
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
                <input type="checkbox" checked={!!form.emergency_services} onChange={(e) => set("emergency_services", e.target.checked)} className="w-4 h-4 rounded border-gray-300" />
                24/7 Emergency Services
              </label>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
                <input type="checkbox" checked={!!form.parking_available} onChange={(e) => set("parking_available", e.target.checked)} className="w-4 h-4 rounded border-gray-300" />
                Parking Available
              </label>
              <label className="flex items-center gap-2 text-sm font-medium text-gray-700">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 p-4 sm:p-5 shadow-sm">
          ...
          <div className="space-y-3">
            <Field label="Contact Email">
              <Input value={form.contact_email || ""} onChange={(e) => set("contact_email", e.target.value)} placeholder="hospital@example.com" className="h-10 rounded-lg border-gray-200" />
            </Field>
            <Field label="Contact Phone">
              <Input value={form.contact_phone || ""} onChange={(e) => set("contact_phone", e.target.value)} placeholder="+91 98765 43210" className="h-10 rounded-lg border-gray-200" />
            </Field>
            <Field label="Website">
              <Input value={form.website || ""} onChange={(e) => set("website", e.target.value)} placeholder="https://hospital.com" className="h-10 rounded-lg border-gray-200" />
            </Field>
          </div>
        </div>
      </div>
